...
func waitForPostgres(container string, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		err := run("docker", "exec", container, "pg_isready", "-U", "restoflux")
		if err == nil {
			return nil
		}
...
func main() {
	...
	if *license != "" {
		...
		if err == nil {
			...
			replaced := false
			for i, l := range lines {
				if len(l) >= 12 && l[:12] == "VITE_LICENSE_" {
					lines[i] = "VITE_LICENSE_KEY=" + *license
					replaced = true
				}
				if len(l) >= 14 && l[:14] == "VITE_INSTANCE_ID" {
					lines[i] = "VITE_INSTANCE_ID=" + *instance
				}
			}
			if !replaced {
				lines = append(lines, "VITE_LICENSE_KEY="+*license)
			}
			// write back
			f.Close()
	...
	if err := waitForPostgres(container, 2*time.Minute); err != nil {
		...
		if fileExists(initSql) {
			...
			_ = run("docker", "exec", container, "psql", "-U", "restoflux", "-d", "restoflux", "-f", "/tmp/init.sql")
		}
	}
